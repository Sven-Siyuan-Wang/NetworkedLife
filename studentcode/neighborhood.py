...
R = np.zeros((trStats["n_users"], trStats["n_movies"]))

for i in range(trStats["n_ratings"]):
    R[trStats["users"][i]][trStats["movies"][i]] = trStats["ratings"][i]
...
l = 6.71
inverse = np.linalg.pinv(A.transpose().dot(A) + l * np.identity(A.shape[1]))
    ...
    for j in range(R.shape[1]):
        # if R_npred[i][j]==0:
            ...
            if R_error[i][top2]==99:
                R_error[i][top2] = 0
                missing2 = True
            if top1 == top2: # patch a bug where l is all 0s
                missing1 = True
                missing2 = True
...
def predict(movies, users, R_npred):
    return [R_npred[users[i]][movies[i]] for i in range(len(movies))]

print lib.rmse(predict(valStats["movies"], valStats["users"],R_npred), valStats["ratings"])
